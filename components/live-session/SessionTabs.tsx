  ...
  return (
    <div className="bg-white rounded-2xl p-2 shadow-sm border border-gray-100 flex items-center justify-between overflow-x-auto">
        ...
        {tabs.map((tab) => (
          ...
          >
            <div className="flex items-center gap-2">
              {tab.label}
              {tab.count && (
                <span className={`px-2 py-0.5 rounded-md text-[11px] font-semibold ${activeTab === tab.id ? 'bg-yellow-700 text-white' : 'bg-gray-100 text-gray-500'
                  }`}>
                  {tab.count}
                </span>
              )}
      ...
      <div className="pr-4 hidden md:block">
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider leading-none">Session Status</p>
      </div>
    </div>
