          ...
          <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="pt-4 border-t border-gray-200">
                ...
                {selectedProject.status !== ProjectStatus.PENDING_APPROVAL ? (
                  // Show existing feedback for non-pending projects
                  <div className={`p-4 rounded-lg ${
                    selectedProject.status === ProjectStatus.APPROVED ? 'bg-green-50 border border-green-200' :
                    selectedProject.status === ProjectStatus.REJECTED ? 'bg-red-50 border border-red-200' :
                    'bg-gray-50 border border-gray-200'
                  }`}>
                    <p className={`text-sm ${
                      selectedProject.status === ProjectStatus.APPROVED ? 'text-green-700' :
                      selectedProject.status === ProjectStatus.REJECTED ? 'text-red-700' :
                      'text-gray-700'
                    }`}>
