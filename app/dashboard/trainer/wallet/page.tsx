...
function computeNextPayoutDate(): string {
  const now = new Date();
  // Next payout is on the 25th of the current or next month
  const payoutDay = 25;
  ...
  return (
    <div className="animate-pulse">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {[1, 2, 3].map((i) => (
          <div key={i} className="bg-gray-200 rounded-lg h-36" />
        ))}
      </div>
      <div className="bg-gray-200 rounded-lg h-24 mb-8" />
...
export default function TrainerWalletPage() {
  const { userWallet, getUserTransactions, getMonthlyRevenue, isLoading } = useFinancial();
  ...
  const formatAmount = (type: string, amount: number) => {
    ...
    return `${prefix}RWF ${amount.toLocaleString()}`;
  };
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-100">
                  ...
                  {transactions.length === 0 ? (
                    <div className="p-12 text-center text-gray-500">
                      ...
                      <p className="text-sm mt-1">Your earnings and withdrawals will appear here</p>
                    </div>
                  ) : (
