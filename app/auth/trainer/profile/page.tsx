        ...
        <div className="flex flex-col flex-1 max-w-md mx-auto w-full">
          ...
          <div className="flex flex-col items-center justify-center flex-1">
            ...
            <p className="text-gray-500 mb-8 text-center text-sm px-4">
              Introduce your expertise and teaching approach to potential learners.
            </p>

            {/* Form */}
            <form onSubmit={handleContinue} className="w-full space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2"> Your Professional Bio</label>
                <textarea
                  value={bio}
                  onChange={(e) => {
                  ...
                  className={`w-full px-4 py-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-600 focus:border-transparent text-gray-900 placeholder:text-gray-400 resize-none h-40 ${error ? 'border-red-500' : 'border-gray-300'
                    }`}
                  required
                />
                <div className="flex justify-between items-center mt-2">
                  {error && <p className="text-xs text-red-500">{error}</p>}
                  <p className="text-xs text-gray-500 ml-auto font-medium">
                    {bio.length} characters (min 50)
                  </p>
                </div>
              </div>
